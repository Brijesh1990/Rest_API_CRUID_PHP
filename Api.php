		...
		switch($_GET['apicall']){
			...
			case 'signup':
				if(isTheseParametersAvailable(array('username','email','password','gender'))){
					...
					if($stmt->num_rows > 0){
						...
						if($stmt->execute()){
							$stmt = $conn->prepare("SELECT uid, uid, username, email, gender FROM users WHERE username = ?"); 
			...
			case 'login':
				...
				if(isTheseParametersAvailable(array('username', 'password'))){
					...
					$stmt = $conn->prepare("SELECT uid, username, email, gender FROM users WHERE username = ? AND password = ?");
